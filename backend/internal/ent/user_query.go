...
func (_q *UserQuery) QuerySchool() *SchoolQuery {
...
func (_q *UserQuery) QueryVoteItems() *VoteItemQuery {
...
func (_q *UserQuery) WithSchool(opts ...func(*SchoolQuery)) *UserQuery {
	query := (&SchoolClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithVoteItems(opts ...func(*VoteItemQuery)) *UserQuery {
	query := (&VoteItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withVoteItems; query != nil {
		if err := _q.loadVoteItems(ctx, query, nodes,
			func(n *User) { n.Edges.VoteItems = []*VoteItem{} },
			func(n *User, e *VoteItem) { n.Edges.VoteItems = append(n.Edges.VoteItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadSchool(ctx context.Context, query *SchoolQuery, nodes []*User, init func(*User), assign func(*User, *School)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "school_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UserQuery) loadVoteItems(ctx context.Context, query *VoteItemQuery, nodes []*User, init func(*User), assign func(*User, *VoteItem)) error {
	...
	query.Where(predicate.VoteItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VoteItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_vote_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_vote_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_vote_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
