...
### Requirement: 前端登录页面
系统 SHALL 在 `/auth/login` 提供登录页面，支持邮箱 + 密码表单，登录成功后将 token 写入 localStorage 并跳转回来源页。

#### Scenario: 登录成功跳转
- **WHEN** 用户在 `/auth/login?next=/admin/session` 提交正确凭据
- **THEN** 前端将 access_token 和 refresh_token 写入 localStorage，执行 `router.push(next ?? '/')`

#### Scenario: Token 过期自动重定向
- **WHEN** 前端发起 API 请求收到 HTTP 401
- **THEN** 前端清除 localStorage 中的 access_token 和 refresh_token，执行 `router.push('/auth/login?next=' + encodeURIComponent(location.pathname + location.search))`

### Requirement: 管理员导航入口
前端 SHALL 在全局顶栏中，仅对 `school_admin` 和 `super_admin` 角色显示"管理后台"导航链接。

#### Scenario: 管理员已登录
- **WHEN** 当前用户 `role` 为 `school_admin` 或 `super_admin`
- **THEN** 全局顶栏显示指向 `/admin/session` 的"管理后台"链接

#### Scenario: 普通用户或未登录
- **WHEN** 当前用户 `role=voter` 或未登录
- **THEN** 顶栏不显示"管理后台"链接

### Requirement: 退出登录
前端 SHALL 在全局顶栏为所有已登录用户提供退出按钮，点击后清除本地凭据并跳转登录页。

#### Scenario: 点击退出
- **WHEN** 已登录用户点击退出按钮
- **THEN** 前端执行 `localStorage.removeItem('access_token')` 和 `removeItem('refresh_token')`，跳转至 `/auth/login`
