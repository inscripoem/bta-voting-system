...
### Requirement: 注册页面前端流程
前端 `/auth/register` 页面 SHALL 根据当前登录状态展示不同内容，引导 Guest 用户走邮件升级流程。

#### Scenario: Guest 用户访问注册页
- **WHEN** 已持有 access_token（Guest 状态）的用户访问 `/auth/register`
- **THEN** 页面展示邮箱验证表单，依次调用 `POST /api/v1/auth/send-code` → `POST /api/v1/auth/verify-email` → `POST /api/v1/auth/upgrade` 完成升级，成功后跳转至 `/`

#### Scenario: 未登录用户访问注册页
- **WHEN** 无 token 的用户访问 `/auth/register`
- **THEN** 页面提示"请先完成投票验证以创建账户"，并显示"去投票"链接（指向 `/vote`）
